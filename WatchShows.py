...
def link_to_use():
	...
	if show_name.endswith("all eps"):
		...
		pass
	search_results = google.search("mywatchseries .to " + show_name) #in case WatchSeries changes their URL, this might need modifications
	for url in search_results:
		link_found = url.link
		break
	if "season" in link_found:
		link_found, season = link_found.rsplit('/', 1)
		link_found = link_found.replace('.to/', '.to/serie/') #in case WatchSeries changes their URL, this might need modifications
		return link_found
	else:
		return link_found
